...
#define CAM_ISP_PACKET_META_MAX (CAM_ISP_PACKET_META_GENERIC_BLOB_COMMON + 1)

#define CAM_ISP_GENERIC_BLOB_TYPE_MAX (CAM_ISP_GENERIC_BLOB_TYPE_BW_CONFIG + 1)
...
static int cam_ife_hw_mgr_reset_csid_res(struct cam_ife_hw_mgr_res *isp_hw_res)
{
	int i;
	int rc = 0;
	...
	for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
		...
		CAM_DBG(CAM_ISP, "Resetting csid hardware %d", hw_intf->hw_idx);
		if (hw_intf->hw_ops.reset) {
			rc = hw_intf->hw_ops.reset(
				hw_intf->hw_priv, &csid_reset_args,
...
static int cam_ife_hw_mgr_init_hw_res(struct cam_ife_hw_mgr_res *isp_hw_res)
{
	int i;
	int rc = -1;
	struct cam_hw_intf *hw_intf;
	...
	for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
		if (!isp_hw_res->hw_res[i])
			continue;
		hw_intf = isp_hw_res->hw_res[i]->hw_intf;
		CAM_DBG(CAM_ISP, "enabled vfe hardware %d", hw_intf->hw_idx);
		if (hw_intf->hw_ops.init) {
			rc = hw_intf->hw_ops.init(
				hw_intf->hw_priv, isp_hw_res->hw_res[i],
...
static int cam_ife_hw_mgr_start_hw_res(struct cam_ife_hw_mgr_res *isp_hw_res,
...
static void cam_ife_hw_mgr_stop_hw_res(struct cam_ife_hw_mgr_res *isp_hw_res)
{
	int i;
	struct cam_hw_intf *hw_intf;
	uint32_t dummy_args;
	...
	for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
		...
		if (hw_intf->hw_ops.process_cmd &&
			...
			hw_intf->hw_ops.process_cmd(
				hw_intf->hw_priv,
				CAM_ISP_HW_CMD_STOP_BUS_ERR_IRQ, &dummy_args,
				sizeof(dummy_args));
		}
	}
}
					   ...
					   void *tocken)
{
	...
	if (!tocken) {
		CAM_ERR(CAM_ISP,
			"isp page fault handler called without a valid tocken");
		return;
	}

	list_for_each_entry(ctx, &ife_hw_mgr->used_ctx_list, list) {
		/* IFE out resources */
...
static void cam_ife_hw_mgr_deinit_hw_res(struct cam_ife_hw_mgr_res *isp_hw_res)
{
	int i;
	struct cam_hw_intf *hw_intf;
	...
	for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
		...
		if (hw_intf->hw_ops.deinit)
			hw_intf->hw_ops.deinit(
				hw_intf->hw_priv, isp_hw_res->hw_res[i],
				sizeof(struct cam_isp_resource_node));
	}
}

static int cam_ife_hw_mgr_put_res(struct list_head *src_list,
...
static int cam_ife_hw_mgr_get_res(struct list_head *src_list,
				  struct cam_ife_hw_mgr_res **res)
{
	int rc = 0;
	struct cam_ife_hw_mgr_res *res_ptr = NULL;
	...
	if (!list_empty(src_list)) {
		res_ptr = list_first_entry(src_list, struct cam_ife_hw_mgr_res,
					   list);
...
static int cam_ife_hw_mgr_free_hw_res(struct cam_ife_hw_mgr_res *isp_hw_res)
{
	int rc = 0;
	int i;
	struct cam_hw_intf *hw_intf;
...
static int cam_ife_mgr_csid_stop_hw(struct cam_ife_hw_mgr_ctx *ctx,
...
static int cam_ife_hw_mgr_release_hw_for_ctx(struct cam_ife_hw_mgr_ctx *ife_ctx)
{
	uint32_t i;
	struct cam_ife_hw_mgr_res *hw_mgr_res;
...
static int cam_ife_hw_mgr_put_ctx(struct list_head *src_list,
...
static int cam_ife_hw_mgr_get_ctx(struct list_head *src_list,
				  struct cam_ife_hw_mgr_ctx **ife_ctx)
{
	int rc = 0;
	struct cam_ife_hw_mgr_ctx *ctx_ptr = NULL;
	...
	mutex_lock(&g_ife_hw_mgr.ctx_mutex);
	if (!list_empty(src_list)) {
		ctx_ptr = list_first_entry(src_list, struct cam_ife_hw_mgr_ctx,
					   list);
...
static void cam_ife_mgr_add_base_info(struct cam_ife_hw_mgr_ctx *ctx,
...
static int cam_ife_mgr_process_base_info(struct cam_ife_hw_mgr_ctx *ctx)
{
	struct cam_ife_hw_mgr_res *hw_mgr_res;
	...
	list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_src, list) {
		...
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			cam_ife_mgr_add_base_info(ctx, i, res->hw_intf->hw_idx);
...
static int
cam_ife_hw_mgr_acquire_res_ife_out_rdi(struct cam_ife_hw_mgr_ctx *ife_ctx,
				       ...
				       struct cam_isp_in_port_info *in_port)
{
	...
	for (i = 0; i < in_port->num_out_res; i++) {
		...
		CAM_DBG(CAM_ISP, "i = %d, vfe_out_res_id = %d, out_port: %d", i,
			vfe_out_res_id, out_port->res_type);
		...
		rc = hw_intf->hw_ops.reserve(
			hw_intf->hw_priv, &vfe_acquire,
	...
	if (i == in_port->num_out_res) {
		CAM_ERR(CAM_ISP,
			"Cannot acquire out resource, i=%d, num_out_res=%d", i,
			in_port->num_out_res);
		goto err;
	}
...
static int
cam_ife_hw_mgr_acquire_res_ife_out_pixel(struct cam_ife_hw_mgr_ctx *ife_ctx,
					 struct cam_ife_hw_mgr_res *ife_src_res,
					 struct cam_isp_in_port_info *in_port)
{
	int rc = -1;
		...
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			...
			if (j == CAM_ISP_HW_SPLIT_LEFT) {
				...
				if (ife_src_res->is_dual_vfe) {
					...
					vfe_acquire.vfe_out.dual_slave_core = 1;
				} else {
...
static int
cam_ife_hw_mgr_acquire_res_ife_out(struct cam_ife_hw_mgr_ctx *ife_ctx,
				   struct cam_isp_in_port_info *in_port)
{
	int rc = -EINVAL;
	struct cam_ife_hw_mgr_res *ife_src_res;
	...
	list_for_each_entry(ife_src_res, &ife_ctx->res_list_ife_src, list) {
		...
		case CAM_ISP_HW_VFE_IN_CAMIF:
			rc = cam_ife_hw_mgr_acquire_res_ife_out_pixel(
				ife_ctx, ife_src_res, in_port);
			break;
		case CAM_ISP_HW_VFE_IN_RDI0:
		case CAM_ISP_HW_VFE_IN_RDI1:
		case CAM_ISP_HW_VFE_IN_RDI2:
		case CAM_ISP_HW_VFE_IN_RDI3:
			rc = cam_ife_hw_mgr_acquire_res_ife_out_rdi(
				ife_ctx, ife_src_res, in_port);
			break;
		default:
...
static int
cam_ife_hw_mgr_acquire_res_ife_src(struct cam_ife_hw_mgr_ctx *ife_ctx,
				   struct cam_isp_in_port_info *in_port)
{
	int rc = -1;
	int i;
...
static int cam_ife_mgr_acquire_cid_res(struct cam_ife_hw_mgr_ctx *ife_ctx,
				       struct cam_isp_in_port_info *in_port,
				       uint32_t *cid_res_id,
				       enum cam_ife_pix_path_res_id csid_path)
{
	...
	*cid_res_id = csid_acquire.node_res->res_id;
	...
	ife_ctx->res_list_ife_in.child[ife_ctx->res_list_ife_in.num_children++] =
		cid_res;
...
static int
cam_ife_hw_mgr_acquire_res_ife_csid_ipp(struct cam_ife_hw_mgr_ctx *ife_ctx,
					struct cam_isp_in_port_info *in_port)
{
	int rc = -1;
	int i;
	...
	list_for_each_entry(cid_res, &ife_ctx->res_list_ife_cid, list) {
...
cam_ife_hw_mgr_get_ife_csid_rdi_res_type(uint32_t out_port_type)
{
...
static int
cam_ife_hw_mgr_acquire_res_ife_csid_rdi(struct cam_ife_hw_mgr_ctx *ife_ctx,
					struct cam_isp_in_port_info *in_port)
{
	int rc = -1;
	int i, j;
	...
	for (i = 0; i < in_port->num_out_res; i++) {
		...
		/* get cid resource */
		rc = cam_ife_mgr_acquire_cid_res(
			ife_ctx, in_port, &cid_res_id,
		...
		csid_acquire.res_id = cam_ife_hw_mgr_get_ife_csid_rdi_res_type(
			out_port->res_type);
...
static int cam_ife_hw_mgr_acquire_res_root(struct cam_ife_hw_mgr_ctx *ife_ctx,
					   struct cam_isp_in_port_info *in_port)
{
	int rc = -1;
...
static int
cam_ife_hw_mgr_preprocess_out_port(struct cam_ife_hw_mgr_ctx *ife_ctx,
				   struct cam_isp_in_port_info *in_port,
				   int *pixel_count, int *rdi_count)
{
	int pixel_num = 0;
	int rdi_num = 0;
	uint32_t i;
...
static int cam_ife_mgr_acquire_hw_for_ctx(struct cam_ife_hw_mgr_ctx *ife_ctx,
					  struct cam_isp_in_port_info *in_port,
					  uint32_t *num_pix_port,
					  uint32_t *num_rdi_port)
{
	int rc = -1;
	int is_dual_vfe = 0;
	int pixel_count = 0;
	...
	cam_ife_hw_mgr_preprocess_out_port(ife_ctx, in_port, &pixel_count,
					   &rdi_count);
...
static int cam_isp_blob_bw_update(struct cam_isp_bw_config *bw_config,
				  struct cam_ife_hw_mgr_ctx *ctx)
{
	struct cam_ife_hw_mgr_res *hw_mgr_res;
	struct cam_hw_intf *hw_intf;
	...
	CAM_DBG(CAM_ISP,
		...
		bw_config->usage_type, bw_config->left_pix_vote.cam_bw_bps,
	...
	list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_src, list) {
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			if (hw_mgr_res->res_id == CAM_ISP_HW_VFE_IN_CAMIF)
				if (i == CAM_ISP_HW_SPLIT_LEFT) {
					cam_bw_bps = bw_config->left_pix_vote
							     .cam_bw_bps;
					ext_bw_bps = bw_config->left_pix_vote
							     .ext_bw_bps;
				} else {
					cam_bw_bps = bw_config->right_pix_vote
							     .cam_bw_bps;
					ext_bw_bps = bw_config->right_pix_vote
							     .ext_bw_bps;
				}
			...
			if (hw_intf && hw_intf->hw_ops.process_cmd) {
				bw_upd_args.node_res = hw_mgr_res->hw_res[i];
				...
				rc = hw_intf->hw_ops.process_cmd(
					hw_intf->hw_priv,
					CAM_ISP_HW_CMD_BW_UPDATE, &bw_upd_args,
...
static int cam_ife_mgr_config_hw(void *hw_mgr_priv, void *config_hw_args)
{
	int rc = -1, i;
	...
	for (i = 0; i < CAM_IFE_HW_NUM_MAX; i++) {
		if (hw_update_data->bw_config_valid[i] == true) {
			rc = cam_isp_blob_bw_update(
				(struct cam_isp_bw_config *)&hw_update_data
					->bw_config[i],
				ctx);
			if (rc)
...
static int cam_ife_mgr_stop_hw_in_overflow(void *stop_hw_args)
{
	...
	cam_ife_mgr_csid_stop_hw(ctx, &ctx->res_list_ife_cid, master_base_idx,
	...
	cam_ife_mgr_csid_stop_hw(ctx, &ctx->res_list_ife_csid, master_base_idx,
				 CAM_CSID_HALT_IMMEDIATELY);
				  ...
				  enum cam_vfe_bw_control_action action)
{
	...
	list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_src, list) {
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			hw_intf = hw_mgr_res->hw_res[i]->hw_intf;
			if (hw_intf && hw_intf->hw_ops.process_cmd) {
				bw_ctrl_args.node_res = hw_mgr_res->hw_res[i];
				    ...
				    uint32_t hw_idx)
{
	...
	for (i = 0; i < CAM_VFE_HW_NUM_MAX; i++) {
		...
		vfe_hw_intf->hw_ops.reset(vfe_hw_intf->hw_priv, &vfe_reset_type,
					  sizeof(vfe_reset_type));
		break;
	}
...
static int cam_ife_mgr_start_hw(void *hw_mgr_priv, void *start_hw_args)
{
	...
	CAM_DBG(CAM_ISP, "INIT IFE CID ... in ctx id:%d", ctx->ctx_index);
	/* INIT IFE CID */
	list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_cid, list) {
	...
	CAM_DBG(CAM_ISP, "INIT IFE csid ... in ctx id:%d", ctx->ctx_index);
	...
	/* INIT IFE SRC */
	CAM_DBG(CAM_ISP, "INIT IFE SRC in ctx id:%d", ctx->ctx_index);
	list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_src, list) {
		rc = cam_ife_hw_mgr_init_hw_res(hw_mgr_res);
		if (rc) {
	...
	/* INIT IFE OUT */
	CAM_DBG(CAM_ISP, "INIT IFE OUT RESOURCES in ctx id:%d", ctx->ctx_index);
	...
	list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_src, list) {
		...
		case CAM_ISP_HW_VFE_IN_RDI3:
			if (!res_rdi_context_set) {
				...
				CAM_INFO(CAM_ISP,
					 "res_id:%d is_rdi_only_context:%d",
					 hw_mgr_res->res_id,
					 ctx->is_rdi_only_context);
			}
			break;
	...
	return 0;
err:
	stop_hw_method.hw_stop_cmd =
		(enum cam_isp_hw_stop_cmd)CAM_CSID_HALT_IMMEDIATELY;
...
static int cam_ife_mgr_release_hw(void *hw_mgr_priv, void *release_hw_args)
{
	int rc = 0;
	...
	CAM_DBG(CAM_ISP, "Exit...ctx id:%d", ctx->ctx_index);
	cam_ife_hw_mgr_put_ctx(&hw_mgr->free_ctx_list, &ctx);
	return rc;
}

static int
cam_isp_blob_hfr_update(uint32_t blob_type,
			...
			struct cam_hw_prepare_update_args *prepare)
{
	...
	for (i = 0; i < hfr_config->num_ports; i++) {
		...
		if ((kmd_buf_info->used_bytes + total_used_bytes) <
		    kmd_buf_info->size) {
			kmd_buf_remain_size =
				kmd_buf_info->size -
				(kmd_buf_info->used_bytes + total_used_bytes);
		} else {
			CAM_ERR(CAM_ISP, "no free kmd memory for base %d",
		...
		rc = cam_isp_add_cmd_buf_update(
			hw_mgr_res, blob_type, blob_type_hw_cmd_map[blob_type],
			blob_info->base_info->idx, (void *)cmd_buf_addr,
			kmd_buf_remain_size, (void *)port_hfr_config,
			&bytes_used);
		if (rc < 0) {
			CAM_ERR(CAM_ISP,
...
static int
cam_isp_blob_clock_update(uint32_t blob_type,
			  struct cam_isp_generic_blob_info *blob_info,
			  ...
			  struct cam_hw_prepare_update_args *prepare)
{
	...
	CAM_DBG(CAM_ISP, "usage=%u left_clk= %lu right_clk=%lu",
		...
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			if (hw_mgr_res->res_id == CAM_ISP_HW_VFE_IN_CAMIF)
				if (i == CAM_ISP_HW_SPLIT_LEFT)
					...
					clk_rate = clock_config->right_pix_hz;
			else if ((hw_mgr_res->res_id >=
				  CAM_ISP_HW_VFE_IN_RDI0) &&
			...
			if (hw_intf && hw_intf->hw_ops.process_cmd) {
				...
				CAM_DBG(CAM_ISP, "res_id=%u i= %d clk=%llu\n",
				...
				rc = hw_intf->hw_ops.process_cmd(
					...
					CAM_ISP_HW_CMD_CLOCK_UPDATE,
					&clock_upd_args,
					sizeof(struct cam_vfe_clock_update_args));
				if (rc)
					       ...
					       uint8_t *blob_data)
{
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_HFR_CONFIG: {
		...
		if (blob_size <
			...
			CAM_ERR(CAM_ISP, "Invalid blob size %u expected %lu",
				blob_size,
				(unsigned long)(sizeof(uint32_t) * 2 +
						sizeof(struct cam_isp_port_hfr_config) *
							hfr_config->num_ports));
			return -EINVAL;
		}

		rc = cam_isp_blob_hfr_update(blob_type, blob_info, hfr_config,
					     prepare);
		if (rc)
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_CLOCK_CONFIG: {
		...
		if (blob_size <
		    (sizeof(uint32_t) * 2 +
		     sizeof(uint64_t) * (clock_config->num_rdi + 2))) {
			CAM_ERR(CAM_ISP, "Invalid blob size %u expected %lu",
				blob_size,
				(unsigned long)(sizeof(uint32_t) * 2 +
						sizeof(uint64_t) *
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_BW_CONFIG: {
		...
		if (blob_size < (sizeof(uint32_t) * 2 +
			...
			CAM_ERR(CAM_ISP, "Invalid blob size %u expected %lu",
				...
				(unsigned long)(sizeof(uint32_t) * 2 +
						(bw_config->num_rdi +
						 2) * sizeof(struct cam_isp_bw_vote)));
			return -EINVAL;
		}
		...
		prepare_hw_data =
			(struct cam_isp_prepare_hw_update_data *)prepare->priv;
					 ...
					 void *prepare_hw_update_args)
{
	...
	rc = cam_packet_util_process_patches(
		prepare->packet, hw_mgr->mgr_common.cmd_iommu_hdl,
	...
	memset(&prepare_hw_data->bw_config[0], 0x0,
	       sizeof(prepare_hw_data->bw_config[0]) * CAM_IFE_HW_NUM_MAX);
	...
	for (i = 0; i < ctx->num_base; i++) {
		...
		if (rc) {
			CAM_ERR(CAM_ISP,
				"Failed in change base i=%d, idx=%d, rc=%d", i,
				ctx->base[i].idx, rc);
			goto end;
		}
		...
		rc = cam_isp_add_io_buffers(
			...
			hw_mgr->mgr_common.img_iommu_hdl_secure, prepare,
			ctx->base[i].idx, &kmd_buf, ctx->res_list_ife_out,
			CAM_IFE_HW_OUT_RES_MAX, fill_fence);

		if (rc) {
			CAM_ERR(CAM_ISP, "Failed in io buffers, i=%d, rc=%d", i,
				rc);
			goto end;
		}
					     ...
					     uint64_t *boot_time_stamp)
{
	...
	list_for_each_entry(hw_mgr_res, &ife_ctx->res_list_ife_csid, list) {
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			hw_intf = hw_mgr_res->hw_res[i]->hw_intf;
			if (hw_intf->hw_ops.process_cmd) {
				csid_get_time.node_res = hw_mgr_res->hw_res[i];
				rc = hw_intf->hw_ops.process_cmd(
					hw_intf->hw_priv,
					CAM_IFE_CSID_CMD_GET_TIME_STAMP,
					&csid_get_time,
					sizeof(struct cam_csid_get_time_stamp_args));
				if (!rc) {
					*time_stamp =
	...
	struct cam_hw_event_recovery_data *ife_mgr_recovery_data)
{
	...
	recovery_data =
		kzalloc(sizeof(struct cam_hw_event_recovery_data), GFP_ATOMIC);
	if (!recovery_data)
		return -ENOMEM;
	...
	rc = cam_req_mgr_workq_enqueue_task(
		task, recovery_data->affected_ctx[0]->hw_mgr,
		CRM_TASK_PRIORITY_0);
 ...
 */
static int
cam_ife_hw_mgr_is_ctx_affected(struct cam_ife_hw_mgr_ctx *ife_hwr_mgr_ctx,
	...
	struct cam_hw_event_recovery_data *recovery_data)
{
	...
	list_for_each_entry(ife_hwr_mgr_ctx, &ife_hwr_mgr->used_ctx_list,
			    list) {
		/*
		...
		if (cam_ife_hw_mgr_is_ctx_affected(
			    ife_hwr_mgr_ctx, affected_core, CAM_IFE_HW_NUM_MAX))
			continue;
		...
		if (recovery_data->no_of_context < CAM_CTX_MAX)
			recovery_data
				->affected_ctx[recovery_data->no_of_context++] =
				ife_hwr_mgr_ctx;
	...
	for (i = 0; i < CAM_IFE_HW_NUM_MAX; i++) {
		...
		CAM_DBG(CAM_ISP, "Vfe core %d is affected (%d)", i,
			recovery_data->affected_core[i]);
	}
	CAM_DBG(CAM_ISP, "Exit");
	return 0;
}

static int cam_ife_hw_mgr_get_err_type(void *handler_priv, void *payload)
{
	...
	list_for_each_entry(isp_ife_camif_res,
		...
		if (core_idx == hw_res_l->hw_intf->hw_idx) {
			status = hw_res_l->bottom_half_handler(hw_res_l,
							       evt_payload);
		}
		...
		if (core_idx == hw_res_r->hw_intf->hw_idx) {
			status = hw_res_r->bottom_half_handler(hw_res_r,
							       evt_payload);
		}
...
static int cam_ife_hw_mgr_handle_camif_error(void *handler_priv, void *payload)
{
	...
	error_status =
		cam_ife_hw_mgr_get_err_type(ife_hwr_mgr_ctx, evt_payload);
 ...
 */
static int cam_ife_hw_mgr_handle_reg_update(void *handler_priv, void *payload)
{
	...
	list_for_each_entry(ife_src_res, &ife_hwr_mgr_ctx->res_list_ife_src,
			    list) {
...
static int
cam_ife_hw_mgr_check_irq_for_dual_vfe(struct cam_ife_hw_mgr_ctx *ife_hw_mgr_ctx,
				      uint32_t core_idx0, uint32_t core_idx1,
...
static int cam_ife_hw_mgr_handle_epoch_for_camif_hw_res(void *handler_priv,
							void *payload)
{
	int32_t rc = -EINVAL;
...
#ifdef VENDOR_EDIT
				if (!epoch_status) {
					if ((ife_hwr_mgr_ctx->crm_sync_mode ==
					     1) &&
					    (ife_hwr_mgr_ctx->frame_count ==
					     0) &&
					    (core_idx == 0)) {
						ife_hwr_mgr_ctx->frame_count =
							1;
	...
	struct cam_vfe_top_irq_evt_payload *evt_payload)
{
	...
	hw_res_l = isp_ife_camif_res->hw_res[0];
	hw_res_r = isp_ife_camif_res->hw_res[1];
	CAM_DBG(CAM_ISP, "is_dual_vfe ? = %d", isp_ife_camif_res->is_dual_vfe);
	...
	case 1:
		...
		CAM_DBG(CAM_ISP, "curr_core_idx = %d, res hw idx= %d", core_idx,
			hw_res_l->hw_intf->hw_idx);
		...
		CAM_DBG(CAM_ISP, "curr_core_idx = %d, ews hw idx= %d", core_idx,
			hw_res_r->hw_intf->hw_idx);
		if (core_idx == hw_res_r->hw_intf->hw_idx) {
...
static int cam_ife_hw_mgr_handle_sof(void *handler_priv, void *payload)
{
	...
	list_for_each_entry(ife_src_res, &ife_hw_mgr_ctx->res_list_ife_src,
			    list) {
		if (ife_src_res->res_type == CAM_IFE_HW_MGR_RES_UNINIT)
			continue;
		...
		case CAM_ISP_HW_VFE_IN_RDI3:
			hw_res = ife_src_res->hw_res[0];
			sof_status = hw_res->bottom_half_handler(hw_res,
								 evt_payload);
...
static int cam_ife_hw_mgr_handle_eof_for_camif_hw_res(void *handler_priv,
						      void *payload)
{
	int32_t rc = -EINVAL;
	struct cam_isp_resource_node *hw_res_l = NULL;
...
static int cam_ife_hw_mgr_handle_buf_done_for_hw_res(void *handler_priv,
						     void *payload)

{
	...
	for (i = 0; i < CAM_IFE_HW_OUT_RES_MAX; i++) {
		...
		if (hw_res_l &&
		    (evt_payload->core_index == hw_res_l->hw_intf->hw_idx))
...
err:
     ...
     * reported.
     */
	rc = cam_ife_hw_mgr_process_overflow(ife_hwr_mgr_ctx, &error_event_data,
	...
	CAM_DBG(CAM_ISP, "Exit buf_done_status Error = %d", buf_done_status);
	return rc;
}
...
int cam_ife_mgr_do_tasklet(void *handler_priv, void *evt_payload_priv)
{
	...
	CAM_DBG(CAM_ISP, "addr of evt_payload = %pK core_index:%d",
		(void *)evt_payload, evt_payload->core_index);
	...
	/* SOF IRQ */
	cam_ife_hw_mgr_handle_sof(ife_hwr_mgr_ctx, evt_payload_priv);

	CAM_DBG(CAM_ISP, "Calling RUP");
	/* REG UPDATE */
	cam_ife_hw_mgr_handle_reg_update(ife_hwr_mgr_ctx, evt_payload_priv);
...
static int cam_ife_hw_mgr_sort_dev_with_caps(struct cam_ife_hw_mgr *ife_hw_mgr)
{
	int i;
...
DEFINE_SIMPLE_ATTRIBUTE(cam_ife_csid_debug, cam_ife_get_csid_debug,
...
static int cam_ife_hw_mgr_debug_register(void)
{
	...
	if (!debugfs_create_file("ife_csid_debug", 0644,
	...
	if (!debugfs_create_u32("enable_recovery", 0644,
	...
	for (i = 0, j = 0; i < CAM_IFE_HW_NUM_MAX; i++) {
		...
		if (!rc) {
			struct cam_hw_info *vfe_hw =
				(struct cam_hw_info *)g_ife_hw_mgr
					.ife_devices[i]
					->hw_priv;
